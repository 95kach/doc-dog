...
import { createDevServer, findFreePort } from '../../src/server/dev-server.js'
...
describe('createDevServer', () => {
  ...
  const appsToClose: ReturnType<typeof createDevServer>['app'][] = []
  ...
  afterEach(async () => {
    await Promise.all(appsToClose.splice(0).map((a) => a.close()))
  ...
  it('exposes an SSE manager with no clients initially', () => {
    const cache = new PageCache()
    const { sse, app } = createDevServer(cache, config)
    appsToClose.push(app)
  ...
  describe('findFreePort', () => {
    it('returns an available port in range', async () => {
      const port = await findFreePort(40000)
      expect(port).toBeGreaterThanOrEqual(40000)
      expect(port).toBeLessThan(40010)
    })

    it('throws when no port is free in the scan range', async () => {
      const servers: net.Server[] = []
      for (let p = 41000; p < 41010; p++) {
        await new Promise<void>((resolve) => {
          const s = net.createServer()
          s.listen(p, () => { servers.push(s); resolve() })
        })
      }
      await expect(findFreePort(41000)).rejects.toThrow('No free port found')
